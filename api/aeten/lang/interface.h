...
#define _AETEN_LANG_IFACE_2(prefix, iface, suffix) prefix##_##iface##_##suffix

#	ifdef AETEN_LANG_METHODS
/* {{{ Method definition */
#	define aeten_lang__method(type, nm, ...) \
	typedef type (*_AETEN_LANG_IFACE(, _##nm##_t))(AETEN_LANG_INTERFACE*, ##__VA_ARGS__); \
	aeten_lang__static_constructor(_AETEN_LANG_IFACE(, _##nm##_constr)) { \
		char *types[] = AETEN_STRING_OF_EACH(type, ##__VA_ARGS__); \
		size_t sizes[] = AETEN_SIZE_OF_EACH(type, ##__VA_ARGS__); \
		_aeten_lang__method_construct(&_AETEN_LANG_IFACE(, i), #nm,  types, sizes); \
	}
	AETEN_LANG_METHODS
