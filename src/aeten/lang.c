...
void _aeten_lang__construct(aeten_lang__interface_t *iface, char const *iface_name, aeten_lang__interface_t *ifc_list[]) {
	...
	iface->parents = (aeten_lang__ParentsList*)aeten_lang__new_instance(aeten_lang__CopyOnWriteArrayList, sizeof(aeten_lang__interface_t), size);
	...
	iface->methods = (aeten_lang__MethodsList*)aeten_lang__new_instance(aeten_lang__CopyOnWriteArrayList, sizeof(aeten_lang__method_definition_t), 2);
}
...
void _aeten_lang__method_construct(aeten_lang__interface_t *iface, char const *name, char *signature_types[], size_t signature_sizes[]) {
	...
	aeten_lang__method_definition_t method = {
		iface,
		name,
		(aeten_lang__Signature*) aeten_lang__new_instance(aeten_lang__CopyOnWriteArrayList, sizeof(aeten_lang__type_t), size)
	};
	for (i=0; i<size; i++) {
